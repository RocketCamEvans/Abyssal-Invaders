...
def move_player():
    ...
    try:
        ...
        turn_based = bool(getattr(req, 'turn_based', False))  # optional flag
        # Get player
        user_db = get_user_db()
        player_data = user_db.get_user(session_id)
        if not player_data:
        ...
        if new_room:
            print(f"[Encounter Check] Room: {new_room.room_id}, visited: {new_room.has_been_visited}, encounter_chance: {new_room.encounter_chance}")
            encounter_triggered = controllers['combat'].check_encounter_chance(new_room)
            print(f"[Encounter Check] Result: {encounter_triggered}")
            
            if encounter_triggered:
                encounter_roll_happened = True
                encounter_roll_result_debug = "encounter_triggered"
                print(f"[Encounter] Generating enemy for floor {player.floor}")
                # Generate enemy for encounter
                enemy_content = controllers['generation'].generate_enemy_content(player.floor, new_room)
                print(f"[Encounter] Enemy content: {enemy_content}")
                enemy = Enemy.create_random_enemy(player.floor, enemy_content['name'], enemy_content['description'])
                print(f"[Encounter] Enemy created: {enemy.name}")
                # Use ally data if we just found one
                ally_for_battle = ally_data if ally_encountered else None
                # Start turn-based battle
                print(f"[Encounter] Starting battle...")
                combat_result = controllers['combat'].start_battle(player, enemy, new_room, ally_for_battle)
                print(f"[Encounter] Battle started, result: {combat_result.get('message', 'no message')}")
